...
public class CommentedThirdPersonController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        */
        ...
        {
            if(Time.time - lastFootstep >= footStep) using UnityEngine;
...
public class CommentedThirdPersonController : MonoBehaviour
    {
        ...
        public float velocity = 5f;
        [Tooltip("This value is added to the speed while sprinting.")]
        public float sprintAdittion = 3.5f;
        [Tooltip("The higher the value, the higher the character will jump.")]
        public float jumpForce = 18f;
        [Tooltip("Stay in the air before falling.")]
        public float jumpTime = 0.85f;

        [Space]
        [Tooltip("Force pulling the player down.")]
        public float gravity = 9.8f;
        ...
        private void FixedUpdate()
        {
            ...
            HandleFootsteps(directionX, directionZ);
        }

        void HandleFootsteps(float directionX, float directionZ)
        {
            if (cc.isGrounded && (directionX != 0 || directionZ != 0))
            {
                float footstepSpeed = isSprinting ? 0.3f : 0.5f;
                if (Time.time - lastFootstep >= footstepSpeed)
                {
                    PlayFootstepSound();
